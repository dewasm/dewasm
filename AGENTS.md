...
- Generated code resolves at conversion time what conversion time knows (decision 96): a name-to-member step over a table that is fixed when the artifact is written is emitted as a literal dispatch, never as a run-time lookup under a computed name.
