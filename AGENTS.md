...
- Hot loop bodies are outlined into per-iteration functions by the shared `dewasm_backend::outline` pass, with thresholds a per-backend judgement (decision 81); a backend adopts it by swapping in the pass's rewritten function list at emission time, never by splitting generated text.
- Constant-address loads are hoisted out of loops by the shared `dewasm_backend::licm` pass, which guards every store in the loop with a runtime address check and reloads on overlap instead of attempting an alias proof (decision 82); it runs before outlining, which removes the loops it needs.
