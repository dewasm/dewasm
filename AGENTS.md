...
| `cargo xtask bench [filter]` | Run the cross-runtime benchmark suite: every backend, wasmtime, and the pywasm/wardite interpreters, over the `benchmarks/` kernels and the SQLite app cases. Writes a dated result file to `benchmarks/results/` and regenerates `docs/benchmarks.md` — a measurement, not a snapshot, so no freshness test guards it. `--list` prints the matrix and each runner's availability without running anything; `--reps`/`--target-ms`/`--timeout` tune the measurement. Needs `wasmtime` plus `benchmarks/kernels/build.sh` and `benchmarks/setup.sh`; anything missing is reported as skipped-with-reason. |
