...
- **A second, orthogonal WASI-support axis** instead of folding WASI p1 into the same scale as wasm-core features.
  Rejected: a backend's wasm- core completeness and its WASI completeness move together in practice (Ruby's decision 16 work touched both), and two axes would need a combination rule to answer "what level is this backend" anyway — one scale with WASI folded in is more direct and matches decision 8's existing practice of listing WASI p1 alongside the `Feature` rows.
