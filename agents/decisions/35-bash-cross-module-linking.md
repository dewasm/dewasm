...
Memory has no single name to flatten to: its derived state (`mem`/`pages`/`max_pages`) is three variables, not one cell.
`MEMORY_EXPORTS` therefore publishes the owning module's *prefix* (`<p>memown`); a re-export just forwards that string, and a consumer's three `declare -gn`s (`${RESOLVED}mem`/`pages`/`max_pages`) still each resolve in one hop.
