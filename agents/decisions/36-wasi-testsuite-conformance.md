...
It is the closest thing to an official WASI conformance suite, and dewasm already produces standalone programs that behave like the `.wasm` they came from (decision 31), so the modules can run through that interface unchanged.
...
- **Vendor the suite as a git submodule**, `tests/wasi-testsuite` on branch `prod/testsuite-base` (the branch carrying the *prebuilt* artifacts under `tests/{c,rust,assemblyscript}/testsuite/wasm32-wasip1/`), mirroring the `tests/spec` submodule.
  ...
  There is no toolchain step to run, the pin is a commit, and updating it is a deliberate commit like `tests/spec`.
...
- **Scope: c + rust + assemblyscript, `wasm32-wasip1` only.**
  The Rust `wasm32-wasip3` tree is excluded (preview 3 is component-model territory, rejected outright by decision 24).
...
- **Known failures are listed with attribution, not implemented now** (decision 8).
  ...
  This is decision 8's contract applied to conformance points: a known failure must be the consequence of a declaration or a tracked defect, never silence.
...
- **A fetch script (like `examples/apps/setup.sh`).**
  That pattern exists for artifacts we rebuild from pinned source or download per-shape (decision 9); this suite ships built `.wasm` we consume verbatim, so a submodule is the lighter, reproducible pin: no build tools, and the version is a commit.
