...
wasm2go's `-embed` flag is the prior art: move the segment bytes into a binary sidecar the program loads at startup, keeping only offset/length constants in the code.
The open question was whether to do this always, and how to shape the sidecar.
...
- **A single in-file base64/blob literal.**
  ...
  A binary sidecar is the actual bytes (mmap-friendly) and leaves the parsed source entirely.
...
The data payload leaves the parsed source; the sidecar is the raw bytes.
