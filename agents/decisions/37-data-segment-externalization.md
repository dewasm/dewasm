...
Honest finding: these interpreters are **code-dominated**, so the source shrinks by roughly `2 × data_bytes − blob` (the eliminated hex) — ~8 MB / 5 % for `ruby.wasm`, not the "dominated by data" the motivating framing assumed.
