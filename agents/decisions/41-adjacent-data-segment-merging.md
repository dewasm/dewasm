...
wasm2go's analogous constant is 4096, but that is an *externalize-only* threshold (bytes moved to a data file, decision 37), a different trade-off; and this pass lives in the core, which cannot see `GenOptions` and so cannot know whether externalization is even on.
...
- Composes with decision 37: `--data-file` externalizes the *merged* blobs, so the data file carries fewer, larger segments and the source fewer prefix-sum constants.
