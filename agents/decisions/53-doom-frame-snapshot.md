...
Add a **deterministic framebuffer snapshot** test, structured like the C-API cases (`crates/dewasm-test-helper/src/apps_capi.rs`) — convert `doom.wasm` in library mode, append per-backend glue, compare output — with three specifics:
