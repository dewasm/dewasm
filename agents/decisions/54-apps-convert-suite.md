...
Add a **whole-cache per-backend conversion suite**: for every backend, convert every cached app and assert the conversion completes with non-empty source, never running the generated program.
...
- Carry-over: the manifest is hand-maintained.
  ...
  Revisit if a backend's conversion cost shifts (e.g. an artifact pin bumps its size across the ~2 s line).
