...
Perl is dynamically typed like Ruby/Python, but its scalars are *not* bignums: an SV holds a 64-bit IV/UV or an NV (double), and the interpreter switches representations silently.
Three measured behaviors shaped the numeric lowering:

1. **Arithmetic takes an integer fast path.**
   For integral operands, `+`/`-`/`*`/`/` compute integer-exact results (beyond double precision: `0.0 + 2^53+1` stays `2^53+1`) and turn `-0.0` operands into `+0`.
   Conversely `use integer` arithmetic reads the raw 64-bit slots and wraps mod 2^64, `/`/`%` become C (truncating, dividend-signed) division, and `>>` becomes arithmetic.
...
- **Native float operators (`$a + $b`) for f64 arithmetic.**
  ...
  The helper + `pack 'd'` round-trip restores IEEE semantics for ~one extra opcode per op.
- **NV (float) division for i64 div_u.**
  Loses low bits for quotients above 2^53; the exact remainder-adjusted idiom costs one extra `%`.
- **Rescuing real runaway recursion instead of counting.**
