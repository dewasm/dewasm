# ADR-66 — `agents/` for Agent-Facing Documents, `docs/` for Human-Facing Ones

Status: **Accepted, 2026-08-11.**
Landed: `docs/adr/` → `agents/decisions/`, `docs/docs-policy.md` → `agents/docs-policy.md`, the `adr-author` skill slimmed to a pointer at `agents/decisions/README.md` § "Adding a new ADR", and the status vocabulary closed to `Accepted` / `Superseded (ADR-N)`.
The entries keep their `ADR-N` identifiers.
...
Both lived under `docs/`, so the layout said nothing about who a file was for, and the rule that separates them (only agent-facing documents cite a decision record) had to be carried in prose against a directory structure that contradicted it.
...
- The citation rule is stated as a directory rule: only files under `agents/` and `AGENTS.md` cite a decision record.
...
- A status label is exactly `Accepted` or `Superseded (ADR-N)`, the parenthetical required.
  `Proposed` is retired: a decision is recorded when it is made, and a tentative idea lives in an issue until then.
  Scope and progress qualifiers ("Ruby only", "relay protocol superseded by ADR-58") go in the Status paragraph, which can say why.
...
- **Rename the `ADR-N` identifiers to match the new path.**
  They are opaque historical identifiers, not descriptions of a location.
  A mass rename buys internal consistency at the cost of breaking every cross-reference already written in issues, pull requests, and commit messages.
