...
- **Provider protocol.**
  A value in the imports table is either a Hash (name → callable, unchanged) or a *provider*: an object with `import(name) → callable | nil` and optionally `attach(instance)`.
  Generated `initialize` calls `attach` once the instance is fully constructed, before the start function (and thus before any wasm code can invoke an import).
  Passing the *instance* rather than just the memory is Node's bind step generalized to cover wasm2c's power (providers can reach exports too).
...
- **The bundled WASI is constructed only when needed**: the first fallback resolution runs `@wasi ||= Rt::WASI.new(args:, env:)`.
  If the embedder covers every WASI import, no instance is created and its side effects (stdio `binmode`) never happen.
