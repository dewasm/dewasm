...
Store masks are the largest group of what remains (a folded expression tree carries one final mask at its store; a hot loop re-masks its counter every iteration), so the within-tree stage left them all in place: 14.6k of the 21.2k `& 0xffffffff` sites in the converted `sqlite3-shell` end a local or temp assignment.
...
Decision 77's constant-equality rewrite also reasons about raw intervals, so a backend holding a per-function `Elision` routes the rewrite and the rendering through that same analysis: the two must agree on which masks drop, or a pinned comparison would read a raw value the pinning never accounted for.
...
- **Keep stores as observation points (decision 71 status quo).**
  Store masks are the largest group of remaining sites (14.6k of the 21.2k on `sqlite3-shell` end a local or temp assignment) and include every loop-carried re-mask; the within-tree stage cannot reach them by design.
