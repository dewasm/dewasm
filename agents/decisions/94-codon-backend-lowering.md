...
- Carry-over: the shared perf passes (decisions 81-83, 90), the deep-crossing flat dispatch (decision 60's criterion), an unboxed same-module `call_indirect` fast path, and the giant-artifact `-release` compile cost are tracked in issue #311.
  CI builds debug throughout (compile cost; the emission-level NaN quieting keeps the semantics optimizer-independent), so the `-release` spec sweep (`DEWASM_CODON_RELEASE=1`) belongs to the local pre-release verification alongside the `ultra_slow_test` cases.
