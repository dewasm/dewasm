...
- **`crates/dewasm/tests/`**: black-box tests of the `dewasm` binary's option handling (`module_name.rs`, `data_file.rs`, `dwarf_line.rs`), spawning it via `CARGO_BIN_EXE_dewasm`.
