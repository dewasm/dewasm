...
#   cache/*.wasm the kernels, via kernels/build.sh
#
...
echo "  wardite $(GEM_HOME="$PWD/$gems" gem list --exact wardite |
  sed -n 's/^wardite (\(.*\))$/\1/p')"
echo
echo "  ruby   benchmarks/drivers/wardite.rb benchmarks/cache/<id>.wasm <n>"
echo "    (with GEM_HOME=$PWD/$gems)"
echo "  python benchmarks/drivers/pywasm.py  benchmarks/cache/<id>.wasm <n>"
