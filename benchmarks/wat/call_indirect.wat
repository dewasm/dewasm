...
;;
;; One iteration makes four indirect calls, each one frame deep, cycling through all four table slots.
;; The callee index is derived from the loop counter so no runtime can devirtualize it.
;; Against call_direct this isolates the dispatch overhead a backend's table representation adds.
;; The module declares a table and an active element segment but executes no table.* instruction, which is the line the pure-Ruby interpreter draws.
;;
;; ---------------------------------------------------------------------------
;; Shared preamble.
