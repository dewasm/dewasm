...
impl Backend for BashBackend {
    ...
    fn generate(&self, module: &Module, opts: &GenOptions) -> Result<Vec<OutputFile>> {
        // Standalone output is a self-contained program: its prefix is fixed, not derived. Library output uses the requested name (lowercased) after validating it.
...
impl<'a> Gen<'a> {
    ...
    fn body(&self, w: &mut CodeWriter) {
        ...
        // WASI units take the state prefix as their first argument; imports are bound to bare command names taking only the guest's arguments, so each bundled syscall gets a per-module wrapper baking the prefix in. The wrapper is `<p>imp_wasi_<name>`, not `<p>wasi_<name>`: the latter is the *unit's* own name once the artifact's runtime carries the prefix, and a wrapper of that name would call itself.
    ...
    /// Snapshot a fragment into a plain variable so repeated textual uses evaluate it once (and so `case $v in` has something to switch on). A fragment that is already a bare variable name passes through unchanged.
...
/// i64; narrow signed loads xor/sub the sign bit, and the i32 ones
/// remask to masked-unsigned.
...
mod units {
    ...
    /// `Embedded` linkage renames the runtime per artifact by rewriting every `rt_`/`mem_`/`wasi_`/`tab_` identifier across the bundle text ([`prefix_runtime_names`]), which is sound only while such a name is always code. One inside a string literal would be rewritten too, silently changing program-visible text (a trap message, an errno key, a path); no unit has one today, and this keeps it that way.
    #[test]
