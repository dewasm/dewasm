//! Bash backend: translates dewasm IR into a sourceable Bash script plus a bundled runtime of `<p>rt_*`/`<p>mem_*`/`<p>tab_*`/`<p>wasi_*` functions carrying the artifact's own prefix, so two generated scripts sourced into one shell never redefine each other's runtime.
//!
//! Lowering conventions:
//! - i32 is a masked-unsigned value in [0, 2^32); i64 is stored as its signed-64 two's-complement bit pattern (bash arithmetic is signed 64-bit only).
//!   Signed/unsigned views are derived inline.
...
//! - Structured control flow maps to `while :; do ...; break; done` wrappers; `br` becomes `break N`/`continue N` (bash counts only loops, `if` adds no level).
//!   Unreferenced labels emit no wrapper.
//! - Functions return values through the globals `R0, R1, ...`; traps set `TRAP_MSG` and propagate status 134 through `|| return $?` chains.
//!   Every generated function ends with an explicit `return 0` because a trailing arithmetic statement would leak status 1.
//! - One module instance per generation-time prefix: functions `<p>f<i>`, state `<p>g<i>`/`<p>mem`/`<p>t<i>` (per unified-index-space table), entry points `<p>init`, `<p>invoke`, `<p>global_get`, and, for `Embedded` linkage, the bundled runtime under the same prefix (`<p>rt_trap`, `<p>mem_i32_load`, ...).
//!   `TRAP_MSG`, `EXIT_CODE`, the `R0..` result registers and `IMPORTS`/`PROVIDERS` stay global: they are the calling protocol two artifacts must share to link at all.
//!   Imported functions resolve from the caller's `IMPORTS` array; other import kinds resolve through `PROVIDERS` + per-kind export maps, with imported globals and imported memory aliased in via `declare -gn` namerefs.
//!
...
/// Emit a top-level shared runtime for the closure of `seeds`, under the flat unprefixed names (`rt_trap`, `mem_i32_load`, ...).
/// This is the `Alias` shape: one runtime several modules generated by [`generate_module_with_units`] share, as the spec harness's `register` path does.
/// `Embedded` output instead carries its own copy under its own prefix ([`prefix_runtime_names`]).
pub fn shared_runtime(seeds: &BTreeSet<String>) -> Result<String> {
    bundler().bundle(seeds, 0)
}

/// Rewrite every runtime function name in a bundle to carry `prefix`, so the artifact's runtime is its own: `rt_trap` -> `<p>rt_trap`, `mem_i32_load` -> `<p>mem_i32_load`.
/// A name is an identifier starting at a non-word character and beginning with one of the four unit scopes: the same grammar the units lint uses to find cross-unit references, which is what makes the rewrite complete.
/// Definitions and call sites are both identifiers, so one pass over the text moves the whole runtime; a units lint keeps these names out of string literals, where a rewrite would change program-visible text.
/// Applied to the bundle only, never to the generated module body (which can be hundreds of megabytes): its call sites are emitted with the prefix already in place.
...
/// Generate one module for `module` with all names prefixed by `prefix` (e.g. `"m1_"`), calling the runtime under its flat unprefixed names: the `Alias` shape, paired with [`shared_runtime`].
/// Returns the source and the set of runtime units it needs; the caller decides whether to bundle them alongside.
...
impl Backend for BashBackend {
    ...
    fn feature_status(&self, feature: Feature) -> SupportStatus {
        match feature {
            // The softfloat covers the full float surface; the harness treats any float skip as a hard failure.
            Feature::Floats => SupportStatus::Supported,
            // Defined tables live in per-index array families keyed by structural type keys, so more than one table lowers directly.
            Feature::MultipleTables => SupportStatus::Supported,
            // Imported globals alias the provider's cell via a `declare -gn` nameref; mutable and immutable both work through it.
            Feature::ImportedGlobals => SupportStatus::Supported,
            // Imported memory aliases the provider's mem/pages/max_pages triplet via `declare -gn` namerefs, keyed off the owning module's flattened prefix; every mem/WASI unit and the inline MemoryGrow/MemorySize lowering work unchanged.
            Feature::ImportedMemories => SupportStatus::Supported,
            // Imported tables alias the provider's array/type-key array/size triplet via `declare -gn` namerefs, keyed off the target table's own base name; tab_init, call_indirect, and active element segments into a shared table all work unchanged through the nameref.
    ...
    fn generate(&self, module: &Module, opts: &GenOptions) -> Result<Vec<OutputFile>> {
        // Standalone output is a self-contained program: its prefix is fixed, not derived.
        // Library output uses the requested name (lowercased) after validating it.
        ...
        // Embedded output carries its own runtime copy under its own prefix, so two generated scripts sourced into one shell define disjoint function sets; an Alias artifact calls the shared flat names instead.
        ...
        out.push_str(&format!(
            "# Requires bash >= 5. Source this file, then call {prefix}init and\n\
             # {prefix}invoke <export> [args...]; results come back in R0, R1, ...\n\
             # Linking (IMPORTS/PROVIDERS) and the runtime prefix: docs/backends/bash.md\n\n"
        ));
        match &opts.runtime {
        ...
        if opts.mode == Mode::Standalone {
            ...
            if wasi_bundled(module, opts.default_wasi, bundler()) {
                ...
                w.line("\tcase \"$1\" in");
                w.line("\t\t--) shift; break ;;");
                w.line("\t\t--dir) shift; [[ $# -gt 0 ]] || { echo \"--dir requires a HOST::GUEST argument\" >&2; exit 1; }; WASI_DIRS+=(\"$1\"); shift ;;");
...
/// The library-mode module name must be a single identifier `[A-Za-z_][A-Za-z0-9_]*`.
/// Bash has no case-carrying namespace (every generated name is a global shell identifier), so the name is *lowercased* to build the prefix; that is the one deliberate mapping left in the product, and it is total and fully specified, unlike the lossy per-backend sanitizers that were removed.
...
/// The state/function prefix of a *validated* module name: `Sqlite3Shell` -> `sqlite3shell_`, `add` -> `add_`.
/// Public so multi-module e2e cases (`compose_modules`) can derive each module's prefix the same way `generate` does.
...
impl<'a> Gen<'a> {
    ...
    fn body(&self, w: &mut CodeWriter) {
        ...
        // WASI units take the state prefix as their first argument; imports are bound to bare command names taking only the guest's arguments, so each bundled syscall gets a per-module wrapper baking the prefix in.
        // The wrapper is `<p>imp_wasi_<name>`, not `<p>wasi_<name>`: the latter is the *unit's* own name once the artifact's runtime carries the prefix, and a wrapper of that name would call itself.
    ...
    fn init(&self, w: &mut CodeWriter) {
        ...
        if has_imports {
            // Guard-declare the two import registries up front: memory resolution (right below) needs PROVIDERS ahead of the func/global import loops that also read it.
            // `declare -gA` never clears an existing array, so a caller's pre-populated IMPORTS (func overrides) / PROVIDERS (per-module export-map prefixes) survive, while an unset one becomes an empty associative array rather than being treated as indexed with its keys evaluated as arithmetic.
            w.line("declare -gA IMPORTS PROVIDERS");
        }
        // Imported memory: resolve the provider's memown prefix and alias its mem/pages/max_pages triplet in via `declare -gn` namerefs, so every mem/WASI/MemoryGrow/MemorySize unit keeps working unchanged whether the memory is owned locally or by another module.
        // `<p>memown` records the flattened owning prefix (not a nameref itself) so a re-export just forwards the string, keeping any further nameref chain at depth 1.
        ...
        if let Some(mem) = &m.memory {
            // Linear memory is an *associative* array, one byte per element: bash indexed arrays are linked lists (O(distance) access), assoc arrays are hash tables (O(1)), which is ~110x faster on the random multi-MB access real apps do.
            // `declare -gA` (never plain `<p>mem=()`, which would create an indexed array) with `=()` also empties any array a re-instantiation left behind.
        ...
        // Imported tables: resolve before the defined tables so both share the unified index space.
        ...
        // The index base keeps room for the imported ones just above.
        ...
        // So the import loop below records whether any fallback happened, and the state is built iff one did, the shell-variable counterpart of the other backends' lazy `@wasi ||=` construction, and the observable the `custom_wasi_provider`/`partial_override` cases turn on.
        // Still eager within `<p>init`: nothing is deferred to the first syscall, so the preopen validation keeps failing instantiation rather than a later call.
        ...
        for (i, import) in m.imported_funcs.iter().enumerate() {
            ...
            if is_wasi_module(&import.module) && self.default_wasi {
                // Fallback order: resolved import -> bundled WASI unit via a per-module prefix wrapper -> ENOSYS stub for known-but-unimplemented syscalls.
        ...
        // Imported globals: resolve the provider's cell and alias it in under the unified index with a `declare -gn` nameref, so reads (`(( x = <p>g<i> ))`), mutable writes (`(( <p>g<i> = v ))`), and init-expr `global.get` offsets all reach the shared cell.
        ...
        for (n, elem) in m.elems.iter().enumerate() {
            // Stage the segment as a temporary element array (function command names, `''` for a `ref.null` item) plus its structural type keys (same convention, `''` for the null items), mirroring the data-segment mem_init staging below.
        ...
        for export in &m.exports {
            if let ExportKind::Global(idx) = export.kind {
                // Flatten to the underlying cell's variable name so a consumer's nameref stays depth <=1: an exported imported global is itself a `<p>g<i>` nameref, so publish its target (`${!<p>g<i>}`); a defined global publishes its own literal variable name.
        ...
        for export in &m.exports {
            if let ExportKind::Table(idx) = export.kind {
                // Flatten to the underlying array's base name, exactly like a global export: an exported imported table is itself a `<p>t<i>` nameref, so publish its target (`${!<p>t<i>}`); a defined table publishes its own literal base name.
    ...
    fn func_type_key(&self, func_idx: u32) -> String {
        type_key(self.module.func_type(func_idx), val_name)
    }

    fn type_key(&self, type_idx: u32) -> String {
        type_key(&self.module.types[type_idx as usize], val_name)
    }
    ...
    fn stmt(&self, w: &mut CodeWriter, stmt: &Stmt, f: &mut Frames) {
        ...
        match stmt {
            ...
            Stmt::MemoryGrow { dst, delta } => {
                ...
                // Evaluate the delta exactly once by snapshotting the candidate page count: a folded `memory.size` delta reads `pages`, which the grow mutates, so a second textual use of the fragment would see the already-updated value.
                // The destination then takes the old size before `pages` is overwritten.
    ...
    /// Effective-address var for an inline mem access: the load/store address with the static offset folded in (base+offset in the unsigned 33-bit range fits bash's signed 64), snapshotted into a plain var so the bounds check and every byte subscript share one canonical decimal key source.
    ...
    /// Inline out-of-bounds check preserving the trap protocol:
    /// compare the effective address against `pages * 65536` (a nameref read for imported memory, so `memory.grow` on the owner is reflected) and cascade status 134 on failure.
    ...
    /// Emit an inline load (address snapshot + bounds check), returning the arithmetic fragment that composes its value directly from memory.
    ...
    fn value(&self, w: &mut CodeWriter, expr: &Expr) -> String {
        ...
        match expr {
            ...
            Expr::Bin(op, a, b) => {
                ...
                // Several inline lowerings textually repeat an operand (e.g. I64ShrU references `b` four times); with folded operands that would blow up geometrically.
                // Snapshot non-trivial operands into a scratch var first (as_var is a no-op for a bare var).
            ...
            Expr::Load { op, addr, offset } => {
                // Snapshot the composed value into a scratch var so callers that repeat the operand don't re-read memory.
                let frag = self.inline_load(w, *op, addr, *offset);
                self.as_var(w, &frag)
            }
    ...
    /// Emit `expr` as one helper command leaving its result in R0, when it is directly command-shaped with pure operands.
    /// Returns false to let the caller take the generic scratch path.
    fn emit_command(&self, w: &mut CodeWriter, expr: &Expr) -> bool {
        match expr {
            ...
            // Loads inline directly into their destination via emit_assign / value(); they never take the R0 command path.
            _ => false,
        }
    }
    ...
    /// Snapshot a fragment into a plain variable so repeated textual uses evaluate it once (and so `case $v in` has something to switch on).
    /// A fragment that is already a bare variable name passes through unchanged.
    ...
    fn arith(&self, expr: &Expr) -> Option<String> {
        match expr {
            ...
            // Floats are their bit patterns: f32 as u32, f64 as the signed-64 pattern (a u64 decimal >= 2^63 would wrap implementation-defined in bash).
...
/// One little-endian byte cell of `<mem>` at effective address `ea` (already a plain integer var): `<mem>[$ea]` for byte 0, `<mem>[$((ea+k))]` for k>0 (the pre-expanded canonical-decimal key the assoc representation needs so it stays on bash's fast subscript path).
...
/// Inline arithmetic reproducing a `mem_<method>` load body: little-endian byte composition with the unit's exact sign-extension form.
/// The i64 top byte `<<56` wraps into the sign bit, which is the signed-64 bit pattern of i64; narrow signed loads xor/sub the sign bit, and the i32 ones remask to masked-unsigned.
...
/// Inline byte-store statements reproducing a `mem_<method>` store body: low byte first, `v >> 8*k & 0xff` per cell.
/// The arithmetic right shift on a negative i64 pattern is harmless: `& 0xff` masks the dragged-in sign bits.
...
fn load_method(op: LoadOp) -> &'static str {
    ...
    match op {
        I32Load => "i32_load",
        I64Load => "i64_load",
        // Floats travel as bit patterns; the integer memory units are exact.
        F32Load => "i32_load",
        F64Load => "i64_load",
...
/// Lint for the bash runtime units: every reference a unit body makes to another unit must be declared in its `# requires:` header, and every unit function must end with an explicit `return`/`:`, since a trailing arithmetic statement would leak status 1 into the `|| return $?` trap cascade.
...
mod units {
    ...
    /// `Embedded` linkage renames the runtime per artifact by rewriting every `rt_`/`mem_`/`wasi_`/`tab_` identifier across the bundle text ([`prefix_runtime_names`]), which is sound only while such a name is always code.
    /// One inside a string literal would be rewritten too, silently changing program-visible text (a trap message, an errno key, a path); no unit has one today, and this keeps it that way.
    #[test]
    ...
    /// Every single-line quoted string on `line`, stopping at a `#` comment outside quotes (bash starts a comment only at a word boundary).
    /// `\` escapes inside double quotes; single quotes have no escapes at all.
