...
/// `SHARED_TABLE`'s driver: `shared_table_a.wat`'s module has no wasm-tier name, so `compose_modules` prefixes it from the case's "TableExp" label (`func_prefix`); `shared_table_b.wat` imports it under the *wasm* module name `"a"` (unrelated to "TableExp"), so PROVIDERS maps that literal key to the exporter's generation prefix (ADR-35).
