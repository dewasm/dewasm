//! Bash end-to-end suites: the shared library / WASI / apps case consts (`dewasm-test-helper`) wired up for the Bash backend. This file holds ONLY the [`BackendUnderTest`] impl, named glue string constants, and per-case macro invocations. Glue is Bash function calls over the R0.. result globals and the `${prefix}mem` byte array, enough to drive the C-API and multi-module cases as well.
...
/// The override/fallback glue: an explicit `fd_write` import wins, `random_get` falls back to the bundled WASI. Captures and prints the actual bytes the module wrote, the same observable proof of interception the other backends' override glues use.
...
/// Two self-contained artifacts sourced into one shell: each carries its own runtime under its own prefix, so `alpha_rt_trap` and `beta_rt_trap` are two functions, not one that the second `source` overwrote. `declare -F` on both names is the structural probe (the bash counterpart of Ruby comparing `Alpha::Rt::Trap` with `Beta::Rt::Trap`), and the trap that follows is raised by Alpha's own copy through the status-134 cascade, with `TRAP_MSG` staying the shared protocol both artifacts write.
...
/// The `wasi_suite!(Bash, Fs, ...)` template: fill `WASI_DIRS` with the one preopen pair, init, invoke `_start`, then surface a `proc_exit` call the same way the standalone main does (`invoke` returns status 133 with the code in `$EXIT_CODE`) as a trailing decimal line, the same observable the Ruby glue's `rescue Prog::Rt::Exit` produces. A case that never calls `proc_exit` just falls off the end of `_start`, so nothing is appended and the script exits 0.
...
/// The root-preopen containment probe's glue: preopen the filesystem root at guest `/` and call the WASI resolver directly (`<p>wasi_resolve_path <p> <dirfd> <path> <follow>`) instead of running a guest, the bash analogue of Ruby's `wasi.send(:resolve_path, ...)`. `follow=1` matches Ruby's `resolve_path`'s `follow_last: true` default.
...
// --------------------------------------------------------------------- C-API drive glue: the same pointer plumbing the other backends write as host-language closures, in Bash's vocabulary: results in the `R0` global, guest memory as the `${P}mem` associative array (one decimal byte per index), and the generated module's own runtime units for the transfers, spelled `"${P}mem_init"` / `"${P}mem_i32_load"`, since an artifact's runtime carries that artifact's prefix and `$P` is the very prefix these drives already carry. No wasmtime snapshot exists (the results live in guest memory), so each drive's output is pinned in the shared case const. Only the file-backed case uses {scratch}.
...
/// The sqlite3 C API driven in memory: `_initialize`, `sqlite3_malloc` + pointer plumbing, open/exec/prepare/step/column/finalize/close. `sqlite3_prepare_v2`'s -1 length is written as its masked-unsigned i32.
...
/// Guest->host callback round trip: the committed `sqlite3-binding.wasm` exports `run_query`, which calls `sqlite3_exec` with a C callback forwarding each row to the *imported* `env.host_row`. The glue provides `host_row` through the `IMPORTS` array (a void import, so it leaves `R0` empty) and collects the rows.
...
/// zeroperl Perl-5.42 eval (issue #67): instantiate the reactor with a zero-returning `env.call_host_function` import stub (only invoked when the guest registers host callbacks, and this program registers none) and a `/dev/null` preopen (`zeroperl_init` returns 1 without it; the Bash runtime accepts a single-file preopen since issue #143), then `_initialize` → `zeroperl_init` → `malloc` + copy a Perl program into guest memory → `zeroperl_eval` → `zeroperl_flush`. The guest program is a quoted heredoc, so its bytes are identical to the other backends'.
...
/// `allocRom`'s buffer via `nes_mem_init`, tick `{frames}` times with no input,
/// compose the frame from agnes's palette-index screen buffer against a 64-entry
/// `\xNN\xNN\xNN` lookup table built once (issue #117; the `& 0x3f` mask is
/// load-bearing), and dump it through the same chunked `printf` pipeline as
/// DOOM. `{rom}` (the cached ROM's host path) and `{frames}` filled by the
/// runner. The trailing `exit 0` matters here in a way it doesn't for DOOM: at
/// 256x240 the pixel count divides the 4096-pixel flush chunk exactly, so the
/// final `[[ -n $fmt ]]` is false and would otherwise leave the script's status
/// at 1 despite a byte-correct frame on stdout.
...
// qjs_repl_pty is wired here because it shares the filesystem cases' standalone QuickJS conversion, though it has no preopens. Ultra: every keystroke re-enters QuickJS's interactive line editor, and each successive evaluation is slower than the last (~135s to the first prompt, ~330s for `[3,1,2].sort()`), which exceeds the shared 180s per-prompt `PTY_TIMEOUT` and timed out on CI (#22).
dewasm_test_helper::qjs_repl_pty_e2e!(Bash, ultra);
// The two language-runtime giants and the packed CRuby run under Bash too (issue #143), all three ultra: CPython's 30 MB wasm becomes a 226 MB script whose one-liner measured ~9.5 min, and CRuby's measured 71 min: a single arithmetic op costs a bash function call under Bash's softfloat. The wasi-vfs-packed CRuby serves its stdlib from guest memory, so it needs no preopens. All three run at `slow` on the other backends, so the apps stay CI-covered.
...
// The C-API cases run under Bash too: a pointer is a decimal in `R0` and guest memory is the `${P}mem` array, which is all the plumbing these drives need, and the NES glue above already does the same in the other direction. The artifacts are within reach as well (41.6 MB libsqlite3, 18.1 MB libpcap, 3.0 MB tree-sitter scripts, all at or below the 45 MB sqlite3-shell script Bash already runs), and all five stay in the `slow` category: 0.7 s tree-sitter, 1.4 s libpcap, 4.0/4.2 s the in-memory sqlite drives, 7.7 s the file-backed one.
...
// The two zeroperl reactor cases (issue #143) are ultra instead. The 25 MB reactor becomes a 229 MB script whose module init alone (the SFS blob carrying the whole Perl core into `zeroperl_mem`) runs 1 m 39 s at ~6 GB of host RSS; the eval case measured 1 m 55 s end to end. ExifTool has no completion evidence at all: a hand-run was still inside `zeroperl_eval` at 69 minutes when it was cut, so its wall time is unknown rather than merely long. Ruby covers it in CI at `slow`.
