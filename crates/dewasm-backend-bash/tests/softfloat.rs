...
use dewasm_backend_bash::find_bash5;
...
fn run_cases(cases: &Cases) {
    ...
    let script_path = dir.join(format!("dewasm-softfloat-driver-{tag}.sh"));
    let input_path = dir.join(format!("dewasm-softfloat-input-{tag}.txt"));
