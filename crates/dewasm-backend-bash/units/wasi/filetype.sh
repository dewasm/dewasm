# wasi_filetype <path>: WASI filetype for <path> via the test builtins, in lstat order (the bash analogue of crates/dewasm-backend-ruby/units/wasi/wasi_filetype.rb, which checks a File::Stat/lstat result).
