...
# WASI path_link: create a hard link <new_path> under <new_fd> to the existing <old_path> under <old_fd>.
# A licensed `--`-guarded `ln -P` (beyond the four mkdir/rmdir/rm/mv commands): `-P` hard-links a symlink source itself rather than its pointee, so a link to a dangling or looping symlink is created without ever following it.
# Both endpoints are resolved with sandbox containment, NOFOLLOW (hard-linking never dereferences a trailing symlink).
# Symlink-following (LOOKUPFLAGS_SYMLINK_FOLLOW on old_flags) is rejected with
# EINVAL (28).
# A missing source is ENOENT (44); a directory source is EPERM
# (63); an existing destination is EEXIST (20); a destination with a trailing slash is ENOENT (44).
wasi_path_link() {
