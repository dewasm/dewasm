...
impl dewasm_test_helper::SpecBackend for CodonSpec {
    ...
    /// Codon compiles each `.wast` file to one program, so the category sizes are set by compile latency, not run time: a plain `cargo test` (the pull-request category, target: ~3 minutes) runs [`FAST_SPEC_FILES`], `--features slow_test` (CI's main-branch run, target: ~5 minutes all in; the shared curated list measured 268 seconds there, over half the target) adds [`SLOW_EXTRA_SPEC_FILES`] and the exception-handling and tail-call files, and the full testsuite runs only under `--features ultra_slow_test`.
