...
//!   `Converted.source` is accumulated at package scope (a `Mutex<String>`,
//!   drained in `assemble`) while only instantiation/assertion statements go in
//!   the body.
...
/// Files `cargo test` runs by default. Go compiles each `.wast` file to one
/// program, so the default gate runs a curated list covering every semantic
/// area (integers, floats, control flow, memory/table, globals, linking, bulk
/// ops) plus the whole ledger; `DEWASM_SPEC_ALL=1` sweeps every file (one
/// `go build` per file — a few seconds each, dominated by compile latency).
...
pub struct GoSpec {
    /// Package-level module declarations (types, constructors, methods) for the
    /// file currently being assembled. Go forbids these inside `func main`, so
    /// they are accumulated here and prepended by `assemble`. The harness runs
    /// files sequentially in one test, so a `Mutex` (needed for `Sync`) never
    /// contends; it is drained at the end of each file.
    decls: Mutex<String>,
}
