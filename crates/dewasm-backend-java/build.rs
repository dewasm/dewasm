//! Embeds the runtime units from runtime/java/units/ as `UNIT_SOURCES: &[(&str, &str)]` (unit id, source).
//! Mirrors the Go crate's build.rs; only the source directory and extension differ.
...
fn main() {
    ...
    let units_dir = Path::new(&manifest_dir).join("../../runtime/java/units");
    let units_dir = units_dir.canonicalize().expect("runtime/java/units exists");
