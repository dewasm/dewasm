...
// WASI_NOTCAPABLE (76) lives in the always-bundled wasi/_class prelude, since the rights model enforces it from stdio-core fd_* units too.
...
// Java's NIO raises typed subclasses of IOException, so match on those; everything else falls back to EIO.
// Note the honest gaps vs the Go/Python backends, which read the raw errno: Java exposes no distinct exception for EISDIR, ELOOP, or ENAMETOOLONG at open/stat time, so those host conditions surface as EIO here unless a syscall detects them itself
