...
int wasi_fd_fdstat_get(int fd, int outPtr) {
    ...
    if (e instanceof Dir) {
        ...
        // A tty reports as a character device (2); a pipe/redirect reports as a regular file (4), so guests' isatty() stays false under piped I/O, matching the wasmtime snapshot captured with piped stdin.
