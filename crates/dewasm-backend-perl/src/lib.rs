    ...
    fn generate(&self, module: &Module, opts: &GenOptions) -> Result<Vec<OutputFile>> {
        ...
        if let Some(cfg) = &opts.data_file {
            if !module.datas.is_empty() {
                ...
                w.indent();
                w.line(format!(
                    "open(my $fh, '<:raw', File::Basename::dirname(__FILE__) . '/' . {}) or die \"dewasm: cannot open data file: $!\\n\";",
                    perl_string(&cfg.data_file_name)
                ));
                w.line("local $/;");
                w.line("scalar <$fh>;");
                w.dedent();
...
impl<'a> Gen<'a> {
    ...
    /// A comparison as a Perl boolean, from a `rel_op` mapping and the already rendered operands.
