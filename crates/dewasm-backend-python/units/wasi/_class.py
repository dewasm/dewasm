...
# Per-fd capabilities are modelled after wasmtime's wasi-common: a directory and a file each carry a different default set, path_open narrows the requested rights against the parent's inheriting set (then per-filetype), and fd_fdstat_set_rights can only drop bits.
# Kept in the always-bundled prelude because __init__ seeds the parallel fd -> [base, inheriting, fdflags] meta map for every preopen and for stdio, so the constants must exist whenever any WASI import is used.
...
# set to the guest-visible path passed in `preopens`) or a directory the guest opened itself via path_open (`preopen_name` None).
# `entries` is the fd_readdir listing cache, populated lazily.
# Nested in the WASI class (so methods reach it as `self.WasiDir`) and kept in the prelude because
# `__init__` builds one per preopen unconditionally, so it must be available whenever any WASI import is used, not only when a filesystem syscall is.
class WasiDir:
...
# Import-provider object: a custom WASI runtime can replace this class wholesale by implementing wasm_import(name) and attach(instance).
