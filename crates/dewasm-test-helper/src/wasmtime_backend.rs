...
//!
//! Public (not gated behind the `wasmtime_test` feature, which only gates the *tests* in `tests/apps_wasmtime.rs`) so that both that test file and `cargo xtask update-repl-snapshot` can drive the same wasmtime-backed [`BackendUnderTest`] to (re)capture the interactive-REPL snapshot.
