...
impl Backend for BashBackend {
    ...
    fn generate(&self, module: &Module, opts: &GenOptions) -> Result<Vec<OutputFile>> {
        if opts.mode == Mode::Standalone {
            bail!(
                "the bash backend does not support --mode standalone yet: WASI and the \
                 standalone entry point follow the integer milestone (ADR-5 sequencing)"
            );
        }
        let prefix = func_prefix(&opts.module_name);
        let (src, units) = generate_module_with_units(module, &prefix, opts.default_wasi)?;
    ...
    fn init(&self, w: &mut CodeWriter) {
        ...
        for (i, import) in m.imported_funcs.iter().enumerate() {
            ...
            if is_wasi_module(&import.module) && self.default_wasi {
                // Bundled WASI is a follow-up milestone; known WASI
                // syscalls fall back to the ENOSYS stub like Ruby's.
