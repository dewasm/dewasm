...
/// written once instead of hand-duplicated per language. Onboarding a new
/// backend is: implement this trait for a unit struct, give each
/// applicable `LibraryCase` a `glues` entry, and write the language's own
/// `#[test]` functions (a few lines each, see `standalone.rs`/`library.rs`).
