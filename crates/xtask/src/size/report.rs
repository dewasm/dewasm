//! The two ends of the size record: the machine-readable file `cargo xtask record-size` writes under `records/` and the `docs/sizes/results.md` `cargo xtask render-size` generates from it.
//!
...
pub fn render_doc(report: &Report, charts: &[Chart]) -> String {
    ...
    out.push_str("# Sizes\n\n");
    out.push_str("<!-- AUTO-GENERATED by `cargo xtask render-size`; do not edit by hand. Unlike docs/support.md there is no freshness test: sizes depend on the host's installed runtimes, so this file is a dated measurement, not a compared snapshot. -->\n\n");
