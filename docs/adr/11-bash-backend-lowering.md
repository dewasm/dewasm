...
- **Structured control flow maps to `while :; do ...; break; done` wrappers; `br` is `break N`/`continue N`.** `if` adds no loop tier in bash, so the generator's label→depth stack stays exact. Unreferenced labels emit no wrapper (ADR-1's `referenced` flag); `br_table` is a `case` (also tier-neutral).
...
- **Dispatch variable for multi-tier `br`** (set a tier flag, re-test after every block) — costs a test per block exit and obscures the code; `break N` maps 1:1 once `if`'s tier-neutrality is established.
