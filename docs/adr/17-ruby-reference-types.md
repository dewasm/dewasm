...
- Positive: full Ruby run pass 29,233 → 29,516; `reference-types` disappeared from the skip histogram; fail count stays 40 with the ADR-16 `import-limits` list byte-identical. Bash's totals are unchanged — the test contained the scope of impact exactly as designed.
