...
Status: **Superseded by [ADR-25](25-retire-support-levels.md), 2026-07-26.** Originally accepted 2026-07-24 and implemented as described below; kept here as history of the tier ladder's design and rationale. ADR-25 removes `Tier` and everything derived from it.
...
- **Tier 4 (Experimental)** — wired into the spec harness (`SpecLang`/`achieved_tier` observes pass/fail/skip) but outside the default test. The starting point for a new backend.
...
- Positive: e2e cases self-describe their requirement instead of being hand-conditional per language; a future language automatically inherits every case its tier covers.
- Negative / caveats: `wasm10_ledger_clean` is a second place (besides the harness's `EXPECTED_FAILURES`) that must be updated when a list entry is cleared — it can drift stale (falsely `false`, which only under-states a tier, never over-states one, so the failure mode is safe but requires remembering to flip it). Tier 1 for Ruby remains unreached: 12 WASI p1 functions (`fd_advise`/`allocate`/`fdstat_set_*`/ `renumber`, `poll_oneoff`, `path_link`/`readlink`/`symlink`, `fd_filestat_set_times`/`path_filestat_set_times`) and the `import-limits` list are the remaining gap.

See also: [ADR-8](8-latest-testsuite-support-matrix.md) (the `Feature` matrix and attribution this builds on), [ADR-15](15-tests-fail-not-skip.md) (the fail-loud policy tier skips deliberately don't fall under), [ADR-16](16-ruby-wasm1-completion.md) (the `import-limits` list gap blocking Ruby's Tier 1).
