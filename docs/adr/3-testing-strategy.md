...
- **Definition of done for a backend = this harness passes.** Adding a language backend means making the shared harness pass for it; no backend-private notion of "works".
...
- **Deviations live in an `EXPECTED_FAILURES` list** in the harness, each entry carrying a count and a reason comment. The file still runs, so regressions in its passing assertions are caught. A list entry is a debt marker, not an exemption: fixing the cause is the default, adding an entry needs the reason written down.
