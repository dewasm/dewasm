...
- **Preopens** arrived through a `DEWASM_PREOPEN` env var (`"guest=host,..."`) parsed in each generated main (~9 sites across the five backends, ADR-14/29/30).
...
wasmtime — the ground-truth engine the app snapshots are captured from (ADR-9/15) — takes `wasmtime run [--dir HOST::GUEST]... <wasm> [args...]` and gives the guest `argv[0] = basename(wasm)`.
...
- **exit:** `proc_exit(N)` → process exit `N`; `_start` returning → `0`; a trap → `trap: <message>` on stderr + exit **134**.
  Bash reaches the same surface via its status-cascade protocol (133 = proc_exit, 134 = trap; ADR-11/12).
...
- **Ignore `--dir` under Bash silently.**
  Violates ADR-0's fail-at-the-boundary rule; a user mounting a directory must be told Bash cannot honor it.
