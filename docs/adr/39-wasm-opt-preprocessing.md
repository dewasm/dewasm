...
- **Skip ripgrep.** ripgrep is a shipping app with a committed wasmtime snapshot, so preprocessing it carries snapshot-drift risk. It is included only because that snapshot is re-verifiable here (wasmtime installed); the rule stays "build it ⇒ eligible, and re-verify."
