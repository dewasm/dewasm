...
Verifying those on every push costs more than it returns, but the fail-loud policy ([ADR-15](15-tests-fail-not-skip.md)) rules out anything that silently skips.
...
- **Runtime time-budget skipping** (skip when a case exceeds N seconds).
  Nondeterministic pass/skip is exactly what ADR-15 forbids; a category flip in a reviewed diff is auditable, a runtime skip is not.
...
- **Keeping the `heavy` name.**
  ...
  Historical ADRs keep the old name; living docs and code use `slow`.
...
- A future case is promoted (or demoted) by editing one callsite and citing a CI run, mirroring the evidence-driven criterion of [ADR-47](47-ruby-f64-sub-quiet-guard.md).
  The DOOM framebuffer snapshot under Bash later joined the ultra-slow category this way ([ADR-53](53-doom-frame-snapshot.md)).
