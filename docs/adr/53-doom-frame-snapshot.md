...
test: drive the converted `doom.wasm` with a self-advancing synthetic clock and
a fixed, input-free tick count, then compare the raw framebuffer it renders —
pixel for pixel — against a snapshot captured once from a wasmtime oracle.
Implemented: the oracle (`cargo xtask update-doom-snapshot`), the shared contract
...
The existing snapshot machinery does not stretch to cover this: the wasmtime
...
- **Driving contract (identical in the oracle and every backend).** Provide the
  ...
  frame.
- **Oracle = the wasmtime crate, in a snapshot generator, not the test path.** A
  small Rust host embeds `wasmtime`, runs the *original* `doom.wasm` under the
  driving contract above, and writes `examples/doom/snapshot/frame.ppm`, refreshed
  on demand via `cargo xtask update-doom-snapshot` (mirroring
  `update-repl-snapshot`). The committed PPM is the oracle the per-backend tests
...
snapshot-testable whenever its output is a deterministic function of an injectable
...
- **Backend-consensus snapshot (no external oracle).** Cheaper — no `wasmtime`
  ...
  embedder is the whole point of a snapshot; rejected.
...
- **Extend the frontends with a snapshot-dump mode instead of dedicated test
  ...
  (synthetic clock, raw dump) into five demo programs and couples the test to
...
- Negative: a new heavy `wasmtime`-crate dependency, quarantined to xtask; the
  snapshot must be regenerated (and reviewed) whenever the `doom.wasm` pin bumps.
- Negative / carry-over: the frame test runs in CI for four backends but Bash's
  ...
  magic constant the snapshot pins — documented at the glue, not derived.
