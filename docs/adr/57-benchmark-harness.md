...
Not covered: any CI enforcement — timings are not reproducible byte-for-byte, so nothing here is a compared snapshot.
...
wardite computes f32 in double precision and never re-rounds, so `f32.add(0.1, 0.2)` yields `0.30000000447034836` — a benchmark that only timed results would have rewarded it for being fast and wrong.
...
wardite sets most of the bound: no f32 (it computes f32 in double precision without re-rounding — silently wrong, not an error), no multi-value, typed `select`, reference types, `table.*` instructions or `data.drop`, and no WASI beyond `args_get`/`args_sizes_get`/`fd_write`/`proc_exit` — imports resolve at instantiation, so merely linking wasi-libc stdio (which imports `fd_seek`) makes a module unloadable there.
