...
- Negative: the `apps` cases fail (per ADR-15) rather than passing vacuously in a fresh clone until the explicit `fetch-and-build.sh` step runs; CI needs that step (or a cached fetch job) before `cargo test` is green. Upstream availability is a fetch-time dependency, mitigated by pinning and checksums.
