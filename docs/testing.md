...
- **Tier rule** (measured, ADR-54): the two giant interpreter artifacts, `ruby` and `cpython`, take >2 s to convert on every backend, so their trials are `#[ignore]`d unless the crate's `slow_test` feature is on; the other eleven run in the fast tier. Un-ignore everything for one backend with `--features slow_test` (or `-- --include-ignored`).
