...
| `examples/doom/golden/frame.ppm` | `cargo xtask update-doom-golden` | `cargo test -p dewasm-backend-ruby --features slow_test --test e2e doom_frame` (Bash needs `--features ultra_heavy_test`) |
...
- **`ultra_heavy_test`** (implies `ultra_slow_test`) — the multi-minute cases above even the ultra_slow band ([ADR-53](adr/53-doom-frame-golden.md)): the DOOM framebuffer golden under Bash (`doom_frame`, `heavy` tier — init ~2 min plus serializing a 1 MB frame out of the associative-array memory). The same case runs at `slow` on Ruby/Python/Go/Java (so DOOM *is* covered in CI); only Bash needs `--features ultra_heavy_test`.
