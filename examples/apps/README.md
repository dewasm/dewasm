...
| sqlite3 | [sqlite 3.53.3 amalgamation](https://sqlite.org/2026/sqlite-amalgamation-3530300.zip), built from source with `zig` | the full SQLite engine in three shapes: the CLI shell, the C-API library, and a guest→host callback binding; the build patches a `-wasm` suffix into the reported version (`3.53.3-wasm`) so converted output identifies itself |
