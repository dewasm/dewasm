...
# fetched or preopened here. We fetch that flattened script from a pinned
# commit and install it as a non-wasm cache tree (cache/exiftool-lib/, the -lib
# convention cpython/cruby use for their stdlib trees). There is no new wasm
# artifact: the exiftool e2e case preopens this script next to the input image
# and drives it through the same embedding C API on the already-cached
# cache/zeroperl.wasm, so the convert suite gains no row.
#
# Licensing: ExifTool is dual-licensed Artistic/GPL ("same terms as Perl
# itself"); the 6over3/exiftool wrapper repo that hosts the flattened script is
# Apache-2.0.

source "$(dirname "${BASH_SOURCE[0]}")/common.sh"

# Pinned commit of the src/exiftool path (resolved via the GitHub commits API,
# audited 2026-08-01) and the sha256 of the flattened script's bytes at it.
