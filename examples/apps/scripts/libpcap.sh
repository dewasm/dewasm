...
pcap_want="$(printf '%s\n%s\n%s' "$PCAP_SHA256" "$(wasm_opt_version)" "$(wasi_sdk_stamp)")"
if is_cached "$pcap_stamp" "$pcap_want" cache/libpcap.wasm; then
  echo "libpcap: cached"
  exit 0
fi

require_wasi_sdk libpcap
...
echo "libpcap: building libpcap.wasm (wasi-sdk clang, reactor)"
psrcs=()
...
wasi_sdk_clang -mexec-model=reactor -O2 \
  ...
  "${psrcs[@]}" src/pcap_wasi/stubs.c src/pcap_binding.c \
