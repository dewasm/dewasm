...
Core modules only: no CPAN installs -- raw mode goes through `stty` because `Term::ReadKey` is not core.
...
Only cells that changed since the previous frame are redrawn, and an SGR code is skipped whenever a cell's color matches the previous cell's -- at this tick rate the diffing is far from necessary, but it's the reference pattern shared with the Ruby/Python/Bash frontends and it keeps a slow link (e.g. ssh) usable.
