...
Only cells that changed since the previous frame are redrawn, and repeated colors within a redrawn run don't re-emit their escape code -- the frame rate is far too low for this to matter, but it's the reference pattern and it keeps a slow link (e.g. ssh) usable.
...
Terminals only report key-down events, never key-up, so a release is synthesized: after a keypress, `reportKeyUp` fires automatically once ~400ms pass without seeing that key again (autorepeat -- holding a key down -- keeps extending the deadline).
