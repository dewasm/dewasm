...
# Terminals only deliver key-down events (no key-up), so releases have to be synthesized; the window is wider than a typical "typematic" gap because dewasm's Python backend only manages
# ~1.3 ticks/sec, so tickGame() calls (and therefore chances to notice a repeat) are ~800ms apart.
RELEASE_TIMEOUT = 0.4
...
# The
# 640x400 framebuffer is a 2x upscale of DOOM's native 320x200, so pixels are read at even (x, y) -- the logical 320x200 grid -- and nearest-neighbor sampled from there down to however many columns/rows actually fit.
