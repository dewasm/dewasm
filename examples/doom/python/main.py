...
CPython interprets doom_gen.py's generated bytecode line by line with no
JIT, so this runs DOOM at roughly 1.3 ticks/sec -- a proof-of-life demo, not
a playable game. See README.md for the honest framing.
"""
import os
import select
import signal
...
# reportKeyDown/reportKeyUp expect the module's KEY_* global values, looked
# up once after the module is constructed (they're plain ints, not globals
...
# A pressed key is held reportKeyDown-active until this many seconds pass
# without seeing it again. Terminals only deliver key-down events (no
# key-up), so releases have to be synthesized; the window is wider than a
# typical "typematic" gap because dewasm's Python backend only manages
# ~1.3 ticks/sec, so tickGame() calls (and therefore chances to notice a
# repeat) are ~800ms apart.
RELEASE_TIMEOUT = 0.4

start_time = time.monotonic()

# Set once the Doom instance exists; the host import closures below need to
# reach its memory, but Python (like Go) needs the closures to exist before
# doom_gen.Doom(...) returns the instance that owns that memory.
doom = None

# Set by loading.onGameInit (640x400 for this binary) and updated by every
# ui.drawFrame call during a tick; never hardcoded.
frame_w = frame_h = 0
frame_off = None
...
# In interactive mode the alternate screen owns the whole terminal, so info
# messages (which Doom's own messages carry no trailing newline for; we add
# one, matching the Go/Java frontends) would corrupt the frame if printed to
# stdout. Routing both message kinds to stderr keeps them visible without
# touching the game's own screen.
...
# Only records where the frame landed; drawFrame can be called mid-tick and
# memory can grow (reallocating the backing bytearray) later in the same
# tick, so the actual pixel read always happens afterwards against a freshly
# fetched memoryview, never a cached one.
def draw_frame(buf_off):
    global frame_off
    frame_off = buf_off


# Leaving both output slots at their pre-zeroed 0 tells the module to fall
# back to its embedded shareware WAD; readWads is then never called at all.
...
# block character, foreground = top pixel, background = bottom pixel. The
# 640x400 framebuffer is a 2x upscale of DOOM's native 320x200, so pixels are
# read at even (x, y) -- the logical 320x200 grid -- and nearest-neighbor
# sampled from there down to however many columns/rows actually fit.
...
def run_smoke():
    ...
    if frame_off is None:
        ...
        # Measure render cost without a real terminal: build the escape
        # string for a representative fixed size and throw it away instead
        # of writing it to a screen.
