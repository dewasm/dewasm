...
// its exported funcs and memory without threading it through every call.
var nesInst *Nes
...
// location rather than the current working directory: build.sh always
// produces ./nes inside examples/nes/go, so walking two directories up from
// there reaches examples/apps/cache regardless of where the binary is
// invoked from (mirroring how build.sh itself locates the repo root via
// `cd "$(dirname "$0")"`).
...
// Unlike DOOM's discrete reportKeyDown/reportKeyUp events, the NES module
// wants the full held-button state on every tick.
...
// loadRom reads the ROM file, copies it into the module's linear memory via
// allocRom, and returns the game-initialized Game ready for the loop.
...
func runSmoke(g *Game) {
	...
	// The NES's PPU palette tops out at 64 colors total, so a healthy frame
	// lands in the dozens; a degenerate (blank/solid) frame lands at 1.
...
func main() {
	...
	// The NTSC NES runs at ~60.0988 Hz; ebiten's default 60 TPS is close
	// enough that no explicit SetTPS call is needed (unlike DOOM's 35Hz,
	// which does need an explicit override).
