...
That builds `libsqlite3.wasm` (needs `zig`), converts it with dewasm, runs the shim + ActiveRecord smokes, generates the Rails app on first use (needs the `rails` gem, >= 8.1), migrates, boots the server, and drives it over HTTP.
