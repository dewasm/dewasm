;; poll_oneoff smoke test: one clock subscription (relative 1 ms) plus an
;; fd_write-readiness subscription on stdout. stdout is always writable, so the
;; call must report at least one event with error 0 without waiting out the
;; timer. On unexpected results the module exits non-zero so failures are loud.
;; Buffers are placed on 8-byte boundaries (poll_oneoff reads/writes i64 fields
;; and hosts such as wasmtime require 8-byte alignment).
(module
  ...
  (func (export "_start")
    ;; --- subscriptions (48 bytes each) at offset 104 ---
    ;; sub[0] @104: clock, monotonic, relative 1 ms.
