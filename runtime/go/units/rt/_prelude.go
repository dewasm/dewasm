...
// resolution type-asserts to that exact type (ADR-7 / ADR-29).
type Imports map[string]any

// ImportProvider is a module source that resolves names on demand, so one
// object can stand in for a whole module — the Go shape of ADR-7's provider
// protocol (Ruby's `import`, Python's `wasm_import`). Returning nil for a
// name leaves that import unresolved, exactly as an absent map entry does, so
// the module still falls back to its bundled WASI / link error.
...
// without the embedder wiring a back-reference by hand (ADR-7).
