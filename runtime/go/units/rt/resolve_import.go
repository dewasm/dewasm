...
// The source under a module name is either a name -> value map or an ImportProvider that resolves names itself.
// Returns nil when the module, or the name within it, is absent, so the caller can fall through to its bundled-WASI / ENOSYS / link-error fallback.
