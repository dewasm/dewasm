...
// (+7 pad), nlink, size, atim/mtim/ctim (all u64, times in nanoseconds). dev/ ino/nlink come from the unix syscall.Stat_t; mtim is the portable ModTime, while atim/ctim are read from the Stat_t's platform time field via reflect
// (the only std, build-tag-free way to reach it, since the field is named Atim on linux and Atimespec on darwin).
// A distinct atim matters for fd_filestat_set_times, which changes mtim while leaving atim untouched.
...
// statTimeNanos reads a syscall.Stat_t timespec field (given its per-platform names) as nanoseconds, via reflect so a single build-tag-free file compiles on both linux (Atim/Ctim) and darwin (Atimespec/Ctimespec).
