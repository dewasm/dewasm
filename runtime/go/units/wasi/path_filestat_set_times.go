...
// Set the access/modification times of a path.
// SYMLINK_FOLLOW selects whether the symlink or its target is affected; os.Chtimes itself follows symlinks, so the NOFOLLOW-on-a-symlink case (no std lutimes) is a known gap.
