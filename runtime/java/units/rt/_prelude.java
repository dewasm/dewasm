...
// exception kinds. Generated code refers to these as `Rt.<name>` /
...
// direct name match (mirroring Go's own deviation from idiom).
interface Fn {
...
// link error. A generated constructor calls `attach` on every provider once
// the instance is fully built, so a provider can reach the instance (its
// memory, above all) without the embedder wiring a back-reference by hand.
