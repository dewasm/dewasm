...
// (never pre-resolved): confinement happens later, when something follows the link (resolve_path's realpath + containment check), not at creation.
// An absolute target is rejected outright: it could only ever escape the preopen tree at follow time.
...
int wasi_path_symlink(int oldPtr, int oldLen, int fd, int newPtr, int newLen) {
    ...
    // A trailing slash on the link path demands the name resolve to a directory, so a symlink can never be created there: report why precisely (ENOENT if nothing is there, EEXIST if a directory is, ENOTDIR if a plain file is).
    if (newRel.endsWith("/")) {
