...
sub wasi_fd_read {
    ...
    for my $i (0 .. $iovs_len - 1) {
        ...
        next if $len == 0;
        # sysread is a raw read(2): it returns as soon as any bytes are available (the WASI short-read semantics wasmtime uses), so an interactive tty stdin (the QuickJS REPL under a pty) never deadlocks waiting for a full buffer.
        my $chunk = '';
