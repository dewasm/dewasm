...
sub wasi_path_link {
    ...
    # WASI must hardlink the source link itself, but macOS link(2) follows a source symlink (Linux's does not) and core perl has no linkat to ask for NOFOLLOW (Ruby binds it via Fiddle).
    # Clone the symlink instead: same target, though a distinct inode/link count, the closest core-perl emulation.
    # An existing destination still reports
