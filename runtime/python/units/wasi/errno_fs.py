...
ERRNO_NOTEMPTY = 55
ERRNO_PERM = 63
# ERRNO_NOTCAPABLE (76) lives in the wasi/_class prelude: rights enforcement in fd_read/fd_write/etc. needs it even when errno_fs is not otherwise bundled.

# One OSError.errno-to-WASI-errno table shared by every filesystem syscall, so the same host error never maps to different codes depending on which syscall raised it.
FS_ERRNO = {
