...
# The host fields are signed (dev_t is signed and macOS reports negative st_dev for pipes and devfs nodes; timestamps can sit before the epoch) and wasmtime copies the bits, so each 64-bit field is masked to u64 rather than letting struct.pack range-check it (issue #132).
