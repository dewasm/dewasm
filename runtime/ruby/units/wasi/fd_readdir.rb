...
def wasi_fd_readdir(fd, buf_ptr, buf_len, cookie, bufused_ptr)
  ...
  # cookie 0 starts a fresh scan, so re-snapshot the listing (a directory
  # mutated between two full readdirs must show the new state); cookie > 0
  # continues the snapshot taken when this scan began.
