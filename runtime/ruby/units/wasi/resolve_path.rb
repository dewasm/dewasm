...
def resolve_path(dirfd, rel, follow_last: true)
  ...
  # An absolute guest path is an escape attempt: File.join would silently collapse the leading slash back inside the base, so reject it outright.
  return [nil, ERRNO_NOTCAPABLE] if rel.start_with?("/")
  base = entry.host_path
  # Lexical escape guard: expand ".." purely textually against the base and reject anything that climbs out. realpath below is the symlink-aware check, but it can't run when the escaped parent doesn't exist on disk
  # (it would raise ENOENT), so a path like "a/../../etc" needs catching here to report NOTCAPABLE rather than NOENT.
